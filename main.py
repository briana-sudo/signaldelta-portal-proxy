...
# ─── STORM/TEMPEST ROUTES MOVED OUT (2026-07-17 service split) ──────────────
# Every storm route (/storm_pull, /spend_solve, /solve_geometry, /address_report,
# the watchlist trio, the promotion trio, /spend_approve, /approve, /push,
# /approve_validate) now lives in KCCStormAPI (storm.signaldeltas.com -> 127.0.0.1:8791,
# repo C:/KCC_Local/storm-engine/stormapi). SignalDeltaProxy no longer fronts TEMPEST, so a
# storm-route change no longer restarts THIS service. The storm read whitelist moved with it;
# the trade /query whitelist below is unchanged. (require_write_bearer + STORM_* constants are
# left defined-but-unused on purpose — harmless, and they document what used to live here.)
