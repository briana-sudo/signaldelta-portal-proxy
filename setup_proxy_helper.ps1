...
if (-not $py) { throw "No python found (looked for $dir\.venv\Scripts\python.exe and PATH)." }
if (-not (Test-Path $nssm)) { throw "nssm not found at $nssm (the proxy service uses it - expected present)." }
if (-not (Test-Path $helper)) { throw "sm_helper.py not found at $helper." }

# a machine-generated token (never shown/sent) shared between helper and proxy
...
if ($ok) {
    ...
    Write-Host "Check $dir\sm_helper.log; you can re-run this setup safely."
}
Write-Host ""
